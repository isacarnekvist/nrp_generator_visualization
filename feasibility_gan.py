...
class GeneratorProduction(torch.nn.Module):
    ...
    def forward(self, obj, obj_, n_samples=10):
        """
        ...
        y = self.rob_norminv(self.generator(self.obj_norm(obj_delta).repeat(n_samples, 1)))
        # generator returns: x, y, cos(θ), sin(θ)
        y[:, 2] = torch.atan2(y[:, 3], y[:, 2])
        # invariant: y[:, :3] = (x, y, θ) is now robot position in object frame
        # convert to robot frame
        y_rob = Variable(torch.zeros(n_samples, 3))
        y_rob[:, 0] = obj[0, 4] + torch.cos(θ) * y[:, 0] - torch.sin(θ) * y[:, 1]
        y_rob[:, 1] = obj[0, 5] + torch.sin(θ) * y[:, 0] + torch.cos(θ) * y[:, 1]
        θr = y[:, 2] + θ
